...
uint256 constant ID = uint256(keccak256("system.NamePet"));

// name pet. assumes operator already created
contract NamePetSystem is System {
  constructor(IWorld _world, address _components) System(_world, _components) {}

  function execute(bytes memory arguments) public returns (bytes memory) {
    (uint256 entityID, string memory name) = abi.decode(arguments, (uint256, string));
  ...
  function executeTyped(uint256 entityID, string memory name) public returns (bytes memory) {
    return execute(abi.encode(entityID, name));
  }
}
